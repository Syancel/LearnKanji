...
def get_onyomi(kanjis):
    ...
    for item in list_onyomi:
        sql = "INSERT INTO onyomi VALUES (" + str(counter) + ",N'" + item.replace("\'", "\'\'") + "')"
        print(sql)
        db.db_exe(sql)
        counter += 1
    return list_onyomi
...
def get_kunyomi(kanjis):
    ...
    for item in list_kunyomi:
        sql = "INSERT INTO kunyomi VALUES (" + str(counter) + ",N'" + item.replace("\'", "\'\'") + "')"
        print(sql)
        db.db_exe(sql)
        counter += 1
    return list_kunyomi
...
def fill_kanji(kanjis, meaning_list, onyomi_list, kunyomi_list):
    ...
    for item in kanjis:
        symbol = item['kanji']
        jlpt = item['jlpt']
        sql_kanji = "INSERT INTO kanji VALUES (" + str(counter) + ",N'" + symbol + "'," + str(jlpt) + ")"
        print(sql_kanji)
        db.db_exe(sql_kanji)
        meaning = item['meanings']
        kunyomi = item['kun_readings']
        ...
        for m in meaning:
            ...
            sql_kanji_meaning = "INSERT INTO kanji_meaning VALUES (" + str(counter_m) + "," + str(counter) + "," + str(meaning_id) + ")"
            print(sql_kanji_meaning)
            db.db_exe(sql_kanji_meaning)
            counter_m += 1
        for k in kunyomi:
            kunyomi_id = kunyomi_list.index(k) + 1
            sql_kanji_kunyomi = "INSERT INTO kanji_kunyomi VALUES (" + str(counter_k) + "," + str(counter) + "," + str(kunyomi_id) + ")"
            print(sql_kanji_kunyomi)
            db.db_exe(sql_kanji_kunyomi)
            counter_k += 1
        for o in onyomi:
            onyomi_id = onyomi_list.index(o) + 1
            sql_kanji_onyomi = "INSERT INTO kanji_onyomi VALUES (" + str(counter_o) + "," + str(counter) + "," + str(onyomi_id) + ")"
